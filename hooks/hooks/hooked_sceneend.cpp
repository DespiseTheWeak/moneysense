...
void __fastcall hooks::hooked_sceneend(void * thisptr, void * edx) {
	...
	if (g_ctx.m_local->is_alive() && g_cfg.antiaim.fakelagchams)
    {
        static auto last_networked_origin = Vector(0, 0, 0);
        if (g_ctx.send_packet)
            last_networked_origin = g_ctx.m_local->m_vecOrigin();
        g_ctx.m_local->set_abs_origin(last_networked_origin);
	...
	//DRAW MENU HERE U GAY OKAY! ok bro
