...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force) {
	...
	static auto draw_animation = []() -> void {
		if (!g_ctx.m_local->is_alive()) return;
	...
	if (panel_id == panel) {
		if (g_ctx.available() && local_player) {
			...
			if (g_cfg.esp.removescope && local_player->m_bIsScoped()) {
				...
				//render::get().text(fonts[SKOPECROSSHAIR], W / 2.0f, H / 2.0f, Color(255, 255, 255, 255), HFONT_CENTERED_X, "J");
				draw::autoskope();
			}
			...
			worldesp::get().DrawZeusRange();
			otheresp::get().bombtimer();
		}
		...
		if (g_cfg.misc.watermark) otheresp::get().watermark();
	}
}
