...
void C_Tab::paint_subtabs() {
	if (subtabs.size()) {
		...
		render::get().rect_filled(area.x, area.y, area.w, 26, Color(20, 20, 20));
		render::get().rect(area.x, area.y, area.w, 27, Color::Black);
		render::get().rect_filled(area.x, area.y + 26, area.w, 4, Color(63, 63, 63));

		render::get().rect_filled(area.x + (sub_width * selected_sub) + 1, area.y + 1, sub_width - 2, 25, Color(31, 31, 31));
...
void C_Tab::paint() {
	...
	render::get().text(fonts[CTAB], tab_area.x + 8, tab_area.y + 10, Color::Grey, HFONT_CENTERED_Y, name.c_str());
	...
	if (index == menu::get().selected_tab) {
		...
		render::get().text(fonts[CTAB], tab_area.x + 8, tab_area.y + 10, g_cfg.menu.menu_theme[0], HFONT_CENTERED_Y, name.c_str());
