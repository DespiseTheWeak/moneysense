...
void C_GroupBox::render() {
	render::get().rect_filled(area.x, area.y, area.w + 10, area.h, Color(17, 17, 17));
	render::get().rect(area.x, area.y, area.w + 10, area.h, Color(17, 17, 17));
	//render::get().rect(area.x - 1, area.y - 1, area.w + 12, area.h + 2, Color::Black);
	render::get().text(fonts[GROUPBOX], area.x + 5, area.y - 1, Color::White, HFONT_CENTERED_Y, name.c_str()); // text ch1ld
}
