...
void C_Dropdown::draw() {
	...
	{
		if (label != "")
			render::get().text(fonts[TAHOMA12], area.x, area.y, Color::FromHSB(rainbow, 1.f, 1.f), HFONT_CENTERED_NONE, label.c_str());
	}
	else
	{
		if (label != "")
	...
	render::get().text(fonts[TAHOMA12], n_area.x + 10, n_area.y + 10, Color(153, 153, 153), HFONT_CENTERED_Y, items[*selected].c_str());
