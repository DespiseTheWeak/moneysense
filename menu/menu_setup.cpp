...
void setup_main_menu() {
	...
	auto rage_tab = new C_Tab(&menu::get(), 0, "A", "Ragebot", {
		...
		auto rage2 = new C_GroupBox(GROUP_RIGHT, 8, "More.."); {
			...
			new C_Dropdown("Resolver Mode", &g_cfg.ragebot.resolver_experimentalmode, { "re$olver", "onetap machine" });
			...
			new C_KeyBind("Fake Duck Key", &g_cfg.misc.fakeduck);
			auto backtracking = new C_CheckBox("Backtracking Exploit", &g_cfg.legit.backtracking);
			auto max_time = new C_SliderFloat("Maximum Backtracking Time", &g_cfg.legit.backtracking_max_time, 0.00f, 0.20f, " s");
			auto visualize = new C_CheckBox("Visualize Backtracking", &g_cfg.legit.backtracking_visualize,
				{ new C_ColorSelector(&g_cfg.legit.backtracking_visualize_color) });
		...
		for (int i = 0; i < 7; i++) {
			set_tabsub(i + 1);
			auto weapon = new C_GroupBox(GROUP_LEFT, 8, "Weapons", 2); {
				for (int s = 0; s < 2; s++) {
					set_sub(s);
					if (s == 0)
					{
		...
		auto antiaim_2 = new C_GroupBox(GROUP_RIGHT, 8, "Other", 3); {
			...
			for (int i = 0; i < 3; i++) {
				...
				else if (i == 1)
				{
					...
					new C_CheckBox("Fakelag", &g_cfg.antiaim.auto_fakelag);
	...
	auto esp_tab = new C_Tab(&menu::get(), 2, "D", "Visuals", {
		...
		auto esp_2 = new C_GroupBox(GROUP_RIGHT, 8, "Other Esp"); {
			...
			new C_CheckBox("Preserve kill feed", &g_cfg.esp.bright);
		...
		auto owo2 = new C_GroupBox(GROUP_RIGHT, 8, "Others");
		{
			...
			auto zeusrange = new C_CheckBox("Draw Zeus Range", &g_cfg.esp.zeusrange);
			//auto timer_bomb = new C_CheckBox("Bomb Timer", &g_cfg.esp.bomb_timer);
	...
	auto misc_tab = new C_Tab(&menu::get(), 3, "G", "Misc"); {
		auto miscellaneous = new C_GroupBox(GROUP_LEFT, 8, "Misc"); {
			...
			auto clantag_spammer = new C_CheckBox("ClanTags", &g_cfg.misc.clantag_spammer);
			new C_Dropdown("ClanTag Modes", &g_cfg.misc.clantag_modes, { "moneysense", "moneybot", "gamesense"});
	...
	auto skinchanger_tab = new C_Tab(&menu::get(), 4, "H", "Changer", {
		...
		auto knifechanger2222222 = new C_GroupBox(GROUP_RIGHT, 12, "Skins");
		{
			...
			auto configs_tab = new C_Tab(&menu::get(), 5, "I", "Settings"); {
				auto config = new C_GroupBox(GROUP_LEFT, 8, "Settings"); {
					...
					new C_CheckBox("Backdrop & Hack Name", &g_cfg.misc.backend);
