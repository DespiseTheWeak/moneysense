...
namespace ResolverOvermake
{
	...
	void Resolver::ResolveDesync(int res, player_t* e) {

		/*for (int i = 1; i < g_csgo.m_globals()->m_maxclients; i++)
		{
			if (e && !e->IsDormant())
			{
				C_BaseEntity::*inputfunc_t;
				auto feet_yaw = yez->GetBasePlayerAnimState()->m_flCurrentFeetYaw;
				auto body_max_rotation = yez->GetBasePlayerAnimState()->pad10[516];
				if (feet_yaw <= 58)
				{
					if (-58 > feet_yaw)
						yez->angs()->y = body_max_rotation + yez->angs()->y;
				}
				else
				{
					yez->angs()->y = body_max_rotation - yez->angs()->y;
				}
				if (yez->GetAnimOverlay(6)->m_flPlaybackRate > 0.1)
				{
					for (int resolve_delta = 58.f; resolve_delta < -58.f; resolve_delta = resolve_delta - 20.f)
					{
						yez->angs()->y = resolve_delta;
					}
				}
			}
		}*/
