...
void eventlogs::paint_traverse() {
	...
	auto name_size = render::get().text_width(name_font, "");
...
void eventlogs::events(IGameEvent * event) {
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_HIT].enabled && strstr(event->GetName(), "player_hurt")) {
		...
		if (attacker_id == g_csgo.m_engine()->GetLocalPlayer()) {
			ss << "You Hit " << userid_info.szName << " In The " << get_hitgroup_name(event->GetInt("hitgroup")) << " For " << event->GetInt("dmg_health");
			ss << " [ " << math::clamp(m_victim->m_iHealth() - event->GetInt("dmg_health"), 0, 100) << " Remaining Health ]" << " Resolvemode: [ " << ResolverOvermake::resolver.TranslateResolveRecord(resolve_type) << " ]";
			add(ss.str(), Color::White);
			ss << "\n";
			fridge::ConsolePrint(ss.str().c_str());
		}
		else if (userid_id == g_csgo.m_engine()->GetLocalPlayer()) {
			ss << attacker_info.szName << "Hit You For " << event->GetInt("dmg_health") << " In The " << get_hitgroup_name(event->GetInt("hitgroup"));
			ss << " You Now Have [ " << math::clamp(m_victim->m_iHealth() - event->GetInt("dmg_health"), 0, 100) << " Remaining Health ] ";
			add(ss.str(), Color::White);
			ss << "\n";
			fridge::ConsolePrint(ss.str().c_str());
		}
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_ITEM_PURCHASES].enabled && strstr(event->GetName(), "item_purchase")) {
		...
		std::stringstream ss;
		ss << userid_info.szName << " Purchased " << event->GetString("weapon");
