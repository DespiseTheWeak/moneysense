...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding - 3);
	window->DrawList->AddRectFilled(ImVec2(total_bb.Min.x - 5, total_bb.Min.y - 5), ImVec2(total_bb.Max.x + 20, total_bb.Max.y + 5), GetColorU32(ImGuiCol_PlotLines), style.FrameRounding - 3);
    if (*v)
    {
        const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
		window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 4,check_bb.Min.y + 4), ImVec2(check_bb.Max.x - 4, check_bb.Max.y - 4), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding - 2);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(frame_bb, id);
   if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Max.y), frame_col, style.FrameRounding, popup_open ? ImDrawCornerFlags_Top : ImDrawCornerFlags_All);

    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button), style.FrameRounding, popup_open ? ImDrawCornerFlags_Top   : ImDrawCornerFlags_All);
		RenderBullet(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y));
    }
    RenderFrame(frame_bb.Min, frame_bb.Max,style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	...
	window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5, grab_bb.Min.y + 7), 12, GetColorU32(ImGuiCol_SliderGrab), 360);
	window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(grab_bb.Max.x, grab_bb.Max.y+2), GetColorU32(/*g.ActiveId == id ? ImGuiCol_SliderGrabActive :*/ ImGuiCol_SliderGrab), style.GrabRounding);


    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    RenderTextClipped(ImVec2(frame_bb.Max.x, frame_bb.Max.y - style.ItemInnerSpacing.y * 3.7f), ImVec2(frame_bb.Max.x * 1.5f, frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(0.1f,0.5f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y - 18), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        draw_list->AddTriangle(tra, trb, trc, IM_COL32(0,0,0,50), 1.5f);
